...
func FuzzLinearMap(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, dMin, dMax, value float64, pMin, pMax float32) {
		...
		got := s.Map(value, pMin, pMax)
		if math.IsNaN(float64(got)) {
			t.Errorf("Map returned NaN for value=%v domain=[%v,%v]",
				value, dMin, dMax)
		}
	})
}

func FuzzLogMap(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, dMin, dMax, base, value float64, pMin, pMax float32) {
		...
		got := s.Map(value, pMin, pMax)
		if math.IsNaN(float64(got)) {
			t.Errorf("Map returned NaN for value=%v domain=[%v,%v] base=%v",
				value, dMin, dMax, base)
		}
		if math.IsInf(float64(got), 0) {
			t.Errorf("Map returned Inf for value=%v domain=[%v,%v] base=%v",
				value, dMin, dMax, base)
		}
	})
}
